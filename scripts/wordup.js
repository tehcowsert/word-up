...
function addNewWordSubmission(word) {
    ...
    console.log(word);
    var wordRepeat = model.wordSubmissions.filter(function(submission){
...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            var currentVal = model.wordSubmissions.length -1;
            console.log(model.wordSubmissions[currentVal])
            if(theAnswer == false){
                model.wordSubmissions[currentVal].isRealWord = false;
            }
            else{
                model.wordSubmissions[currentVal].isRealWord = true;
            }
            // re-render
            render();
        },
...
function render() {
    ...
    $("#word-attempt-form").append(wordSubmission);

    // Set the value of the letterChip
...
function containsOnlyAllowedLetters(word) {
    ...
    if (disallowedLettersInWord(model.currentAttempt).length > 0){
...
function currentScore() {
    ...
    var totalScore = 0;
    totalScore = wordScores;
    return totalScore.reduce(add, 0);
}
